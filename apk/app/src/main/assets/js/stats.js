(function () {
  ...
  function dailyStreak() { const a = attempts(); if(a.length === 0) return 0; const days = new Set(a.map(x => x.attemptedAt.slice(0,10))); let count = 0; const d = new Date(); while(true) { const key = d.toISOString().slice(0,10); if(days.has(key)) { count++; d.setDate(d.getDate()-1); } else { if(count === 0 && key === new Date().toISOString().slice(0,10)) { d.setDate(d.getDate()-1); continue; } break; } } return count; }
  function questionsToday() { const key = new Date().toISOString().slice(0,10); return attempts().filter(x => x.attemptedAt.slice(0,10) === key).length; }
  ...
  window.Stats = { totals, categoryStats, weakestCategories, performanceLabel, dailyStreak, questionsToday, recentSessions, bestScore, formatTime, formatMs };
})();
